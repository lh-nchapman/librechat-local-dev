...
class ModelEndHandler {
  ...
  async handle(event, data, metadata, graph) {
    ...
    try {
      ...
      // === DIAGNOSTIC LOGGING FOR MCP TOOL CALLING ===
      const toolCallCount = Array.isArray(toolCalls) ? toolCalls.length : 0;
      const finishReason = data?.output?.additional_kwargs?.finish_reason ||
                          data?.output?.response_metadata?.finishReason ||
                          'unknown';
      const contentLength = typeof data?.output?.content === 'string'
        ? data?.output?.content.length
        : Array.isArray(data?.output?.content)
          ? JSON.stringify(data?.output?.content).length
          : 0;
      const outputTokens = data?.output?.usage_metadata?.output_tokens || 0;

      // Detect potential premature stop: high token output with no tool calls
      const isPrematureStop = toolCallCount === 0 && outputTokens > 500;

      // Get memory usage
      const memUsage = process.memoryUsage();
      const memoryMB = Math.round(memUsage.heapUsed / 1024 / 1024);

      logger.debug('[ModelEndHandler] Response analysis', {
        conversationId: metadata.thread_id,
        provider: agentContext.provider,
        model: metadata?.ls_model_name || agentContext.clientOptions?.model,
        finishReason,
        toolCallCount,
        ...
        memoryMB,
        isPrematureStop,
        // Include first 200 chars of content if premature stop (helps debug why)
        contentPreview: isPrematureStop && typeof data?.output?.content === 'string'
          ? data.output.content.substring(0, 200) + '...'
          : undefined,
      });

      if (isPrematureStop) {
        logger.warn('[ModelEndHandler] ⚠️ PREMATURE STOP DETECTED', {
          conversationId: metadata.thread_id,
          outputTokens,
          finishReason,
          memoryMB,
          hint: 'Model generated text instead of calling tools. Check context size and tool availability.',
        });
      }
